...
//! Patched encoding of integer arrays. Intended for general purpose encoding,
//! where detailed information about the distribution of the entries is not
//! known. Implemented for all primitive integer types.
//!
//! Slower than the BitPacked encoding, but compression decays more gracefully
//! with the width of the probability distribution of the entries. Often uses
//! one or two bits per integer more than the Shannon limit.
//!
//! # Examples
//!
//! ```
...
/// Patched encoding handles this by performing several initial transformations
/// to reduce the minimum necessary bit width. Specifically, the median value
/// is subtracted from the entries, the entries are mapped to the unsigned
/// integers by the zig-zag encoding, and the most significant bits of any
/// outliers are removed and stored separately. The result is that the
/// compression effectively depends only on the standard deviation of the 
...
impl<B> Access<ops::RangeInclusive<usize>> for Patched<B> where B: Bits {
  type Output = Vec<B>;
...
impl<B> Access<ops::RangeToInclusive<usize>> for Patched<B> where B: Bits {
  type Output = Vec<B>;
    ...
    impl AccessPrivate<ops::Range<usize>> for Patched<$ty> {
      unsafe fn _access(storage: &[u32], range: ops::Range<usize>) -> Vec<$ty> {
        ...
        for _ in 0..(e_blk - s_blk) {
          ...
          let shift: $ty = *(s_ptr as *const $ty);
          s_ptr = s_ptr.offset(utility::words_for_bits(ty_wd) as isize);
          mayda_codec::$dec_zz(o_ptr, 128, shift);
