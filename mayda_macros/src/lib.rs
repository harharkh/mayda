...
//! `decode_zz!` syntax extentions follow the conventions `encode_delta_T`,
//! `encode_shift_T` and `encode_zz_T_shift`. `T` is one of the unsigned
//! integer types, `a` is the number of bits per integer, and `b` is the number
//! of integers encoded.
//!
...
/// Generates encode_delta_T, encode_delta_T and encode_zz_shift_T functions.
fn encode_zz_expand(cx: &mut ExtCtxt,
  ...
  items.push(
    quote_item!(cx,
      pub unsafe fn $ident(i_ptr: *mut $ut, length: usize) {
        ...
        if length >= 2 * $lanes {
          let mut upr = $load(i_slice, length - $lanes);
          for idx in (2..(length / $lanes) + 1).map(|x| length - x * $lanes) {
            let lwr = $load(i_slice, idx);
            let lhs = upr - lwr;
            lhs.store(i_slice, idx + $lanes);
            upr = lwr;
          }

          let mut upr = *i_ptr.offset((length % $lanes + $lanes - 1) as isize);
          for a in (0..((length % $lanes + $lanes - 1) as isize)).rev() {
            let lwr = *i_ptr.offset(a);
            *i_ptr.offset(a + 1) = upr.wrapping_sub(lwr);
            upr = lwr;
          }
        } else {
          let mut upr = *i_ptr.offset((length - 1) as isize);
          for a in (0..(length - 1) as isize).rev() {
            let lwr = *i_ptr.offset(a);
            *i_ptr.offset(a + 1) = upr.wrapping_sub(lwr);
            upr = lwr;
          }
  ...
  // Name for the function interned
  let name = format!("encode_shift_{}", ut);
  let ident = token::str_to_ident(&*name);
  ...
  items.push(
    quote_item!(cx,
      pub unsafe fn $ident(i_ptr: *mut $ut, length: usize, shift: $ut) {
        ...
        for idx in (0..(length / $lanes)).map(|x| x * $lanes) {
          let lhs = $load(i_slice, idx) - simd_shift;
          lhs.store(i_slice, idx);
        }

        let idx = length - length % $lanes;
        for x in i_slice[idx..].iter_mut() {
  ...
  items.push(
    quote_item!(cx,
      pub unsafe fn $ident(i_ptr: *mut $ut, length: usize, shift: $ut) {
        ...
        for idx in (0..(length / $lanes)).map(|x| x * $lanes) {
          let rhs = $load(i_slice, idx) - simd_shift;
...
/// Generates decode_delta_T, decode_shift_T and decode_zz_shift_T functions.
fn decode_zz_expand(cx: &mut ExtCtxt,
  ...
  // Name for the function interned
  let name = format!("decode_delta_{}", ut);
  let ident = token::str_to_ident(&*name);
  ...
  items.push(
    quote_item!(cx,
      pub unsafe fn $ident(o_ptr: *mut $ut, length: usize, shift: $ut) {
        let o_slice = std::slice::from_raw_parts_mut(o_ptr, length);

        *o_ptr = shift;
        let mut acc = shift;

        if length >= 2 * $lanes {
          for a in 1..((length % $lanes + $lanes) as isize) {
            acc  = acc.wrapping_add(*o_ptr.offset(a));
            *o_ptr.offset(a) = acc;
          }

          let mut acc = $load(o_slice, length % $lanes);
          for idx in (1..(length / $lanes)).map(|x| length % $lanes + x * $lanes) {
            acc = acc + $load(o_slice, idx);
            acc.store(o_slice, idx);
          }
        } else {
          for a in 1..(length as isize) {
            acc  = acc.wrapping_add(*o_ptr.offset(a));
            *o_ptr.offset(a) = acc;
          }
        }
  ...
  // Name for the function interned
  let name = format!("decode_shift_{}", ut);
  let ident = token::str_to_ident(&*name);
  ...
  items.push(
    quote_item!(cx,
      pub unsafe fn $ident(o_ptr: *mut $ut, length: usize, shift: $ut) {
        ...
        for idx in (0..(length / $lanes)).map(|x| x * $lanes) {
          let lhs = $load(o_slice, idx) + simd_shift;
          lhs.store(o_slice, idx);
        }

        let idx = length - length % $lanes;
        for x in o_slice[idx..].iter_mut() {
  ...
  items.push(
    quote_item!(cx,
      pub unsafe fn $ident(o_ptr: *mut $ut, length: usize, shift: $ut) {
        ...
        for idx in (0..(length / $lanes)).map(|x| x * $lanes) {
          let rhs = $load(o_slice, idx);
          let xor = zeros - (rhs & ones);
          let lhs = ((rhs >> 1usize) ^ xor) + simd_shift;
          lhs.store(o_slice, idx);
        }

        let idx = length - length % $lanes;
        for x in o_slice[idx..].iter_mut() {
          let xor = (0 as $ut).wrapping_sub(*x & 1);
          *x = ((*x >> 1) ^ xor).wrapping_add(shift);
        }
      }
    ).unwrap()
  );
